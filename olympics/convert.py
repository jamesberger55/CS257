...
for row in reader:
    olympian_id = row[0]
    olympian_name = row[1]
    if olympian_id not in olympian:
...
for row in reader:
    ...
    if olympic_info_year not in olympics_info:
        ...
        olympics_info[olympic_info_year] = olympic_info_id
        writer.writerow([olympic_info_id, olympic_info_year, olympic_info_season, olympic_info_city])
olympics_info_file.close()
kaggle_data.close()

noc_regions = {}
noc_kaggle_file = open('noc_regions.csv')
...
for row in reader:
    ...
    olympic_info_year = row[9]
    olympic_info_id = olympics_info[olympic_info_year] 
    noc = row[7]
    noc_id = noc_regions[noc] 
    medal = row[14]
    writer.writerow([olympian_id, event_id, country_id, game_id, olympic_info_id, noc_id, medal])
